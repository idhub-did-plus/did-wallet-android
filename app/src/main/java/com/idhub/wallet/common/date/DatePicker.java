...
public class DatePicker {
    ...
    private void initDialog() {
        if (datePickerDialog == null) {
            datePickerDialog = new Dialog(context, R.style.WalletDatePickerStyle);
//            datePickerDialog.setCancelable(false);
            datePickerDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
            datePickerDialog.setContentView(R.layout.wallet_common_date_picker);
     ...
     * 设置日期控件是否显示日和时和分
     */
    public void showSpecificTimeNoDay(boolean show) {
        if (canAccess) {
            if (show) {
                disScrollUnit();
